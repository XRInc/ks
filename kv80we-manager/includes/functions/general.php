<?php
function save_product_review($imgUrl)
{
    if (!empty($imgUrl)) {
        list($oldWidth, $oldHeight, $iType) = @getimagesize($imgUrl);

        if (!isset($iType)) return false;
        ...
        //扩展名
        $type = substr($imgUrl,-3, 3);
        //保存路径
        $filePath = DIR_FS_CATALOG_IMAGES . 'review/';
        ...
        $imgFile     = strtotime(date('YmdHis')) . mt_rand(10000,99999) . '.' . $type;
        $imgPath     = $filePath . $imgFile;
        $imgPathFile = DIR_WS_CATALOG_IMAGES . 'review/' . $imgFile;

        $imgcreatePrefix = 'imagecreatefrom' . $imgType;
        $imgPrefix       = 'image' . $imgType;
        $srcImg          = $imgcreatePrefix($imgUrl);
        $newImg = imagecreatetruecolor($oldWidth, $oldHeight);
        imagecopyresampled($newImg, $srcImg, 0, 0, 0, 0, $oldWidth, $oldHeight, $oldWidth, $oldHeight);
        $imgPrefix($newImg, $imgPath);
        imagedestroy($srcImg);
        imagedestroy($newImg);
